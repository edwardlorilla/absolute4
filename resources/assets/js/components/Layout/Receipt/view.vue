<template>
    <div v-if="data" class="dv">
        ...
        <div class="dv-header">
            <div class="dv-header-title">
                <el-select name="division_select" v-model="division_id" @change="changeDivision" id="division_select">
                    <el-option value="">Please select</el-option>
                    <el-option v-for="select in divisions" :value="select.id" :label="select.name" :key="select.id"></el-option>
                </el-select>
                <el-date-picker
                    ...
                    <el-table
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                               {{scope.row.medicine_id ? scope.row.medicine.name : scope.row.name }}
                            </template>
                        </el-table-column>
                        <el-table-column
                                prop="amount"
    ...
    export default {
        ...
        methods: {
            ...
            changeDivision(){
                ...
                var query = {}
                if (vm.division_id) {
                    query.division = vm.division_id
                }
                if (vm.receipt_date) {
                    query.receipt_date = vm.receipt_date
                }
                vm.loading = true
                vm.$router.push({
                    query
            ...
            {
                var vm = this
                if (vm.$route.query.receipt_date) {
                    vm.receipt_date = vm.$route.query.receipt_date
                }
                if (vm.$route.query.division) {
                    vm.division_id = _.parseInt(vm.$route.query.division)
                }
                if (err) {
                    ...
                    vm.divisions = data.data.division;
